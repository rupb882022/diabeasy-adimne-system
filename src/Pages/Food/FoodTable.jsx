...

      {
        ...
        renderCell: (params) => {
          ...
          if (params && params.value) {
            let names = params.value.map(x => x.name)
            list = [...new Set(names)];
            for (let i = 0; i < list.length; i++) {
              for (let j = 0; j < params.value.length; j++) {
                if(params.value[j].name==list[i]){
                  categoryArr.push(params.value[j])
                  j=params.value.length;
                }
              }
            }
          }

          return <ul> {categoryArr.map((x, i) => <li style={{ textAlign: 'left' }} key={i + "p4"} value={x.id}>{x.name}</li>)} </ul>
        }
      },
      {
        name: 'UnitOfMeasure',
